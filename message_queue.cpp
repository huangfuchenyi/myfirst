...
static int _send_message(struct CarDVMsgBuf *qbuf, bool bHighPriority)
{
    ...
    if (msgsnd(g_msgQueueId, (void *)qbuf, sizeof(qbuf->cmdId) + sizeof(qbuf->s32Wait) + sizeof(qbuf->paramLen) + qbuf->paramLen, 0) == -1)
    {
...
RETRY:
    while ((ret = _read_message(&msgBuf)) != -1)
    {
        ...
        case CMD_VIF:
            ret = vif_process_cmd(msgBuf.cmdId, msgBuf.paramBuf, msgBuf.paramLen);
            break;
        ...
        #if CARDV_IE_ENABLE
        case CMD_IE:
            ret = ie_process_cmd(msgBuf.cmdId, msgBuf.paramBuf, msgBuf.paramLen);
            break;
        #endif
        ...
        #if CARDV_TR_ENABLE
        case CMD_TR:
            ret = trafficDetect_process_cmd(msgBuf.cmdId, msgBuf.paramBuf, msgBuf.paramLen);
            break;
        #endif
        }
